...
def cal_acc(loader, fea_bank, socre_bank, netF, netB, netC, args, flag=False):
    ...
    if True:
        ...
        acc = " ".join(aa)
        if True:
            return aacc, acc, Fscore*100  # , acc1, acc2#, nu_mean, s10_avg

    else:
        return accuracy * 100, mean_ent, Fscore*100
...
def train_target(args):
    ...
    score_bank = torch.randn(num_sample, args.class_num).cuda()#########number of classes
    ...
    while iter_num < real_max_iter:
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            if args.dset == "visda-2017":
                ...
                log_str = (
                    "Task: {}, Iter:{}/{};  Acc on target: {:.2f}, Fscore on target: {:.2f}".format(
                        args.name, iter_num, max_iter, acc, Fscore
                    )
                    + "\n"
                    + "T: "
                    + accc
