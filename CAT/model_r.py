...
class LeNetModel(object):
    ...
    def __init__(self, num_classes=2, is_training=True,image_size=9,dropout_keep_prob=0.5):###########2/32
      ...
      self.source_moving_centroid=tf.compat.v1.get_variable(name='source_moving_centroid',shape=[num_classes,self.featurelen],initializer=tf.compat.v1.zeros_initializer(),trainable=False)
      self.target_moving_centroid=tf.compat.v1.get_variable(name='target_moving_centroid',shape=[num_classes,self.featurelen],initializer=tf.compat.v1.zeros_initializer(),trainable=False)
    ...
    def adloss(self,x,xt,y,yt,LAMBDA,weightlamb):
          ...
          with tf.compat.v1.variable_scope('reuse_inference') as scope:
              ...
              target_pred_onehot = tf.compat.v1.one_hot(tf.compat.v1.argmax(target_pred, 1), 10)
              target_pred_soft = tf.compat.v1.nn.softmax(target_pred, 1)
              target_chose = tf.compat.v1.cast(tf.compat.v1.reduce_max(target_pred_soft, 1) > 0.9, tf.bool)
              self.chose_rate = tf.compat.v1.reduce_mean(tf.compat.v1.cast(target_chose, tf.compat.v1.float32))
              target_feature_chose = tf.compat.v1.boolean_mask(target_feature, target_chose)
          ...
          self.source_sntg_loss = tf.compat.v1.reduce_mean(graph_source * distance_source + (1-graph_source)*tf.compat.v1.nn.relu(LAMBDA- distance_source))
          ...
          tf.compat.v1.summary.scalar('JSD',self.G_loss/2+math.log(2))
...
def D(x):
  with tf.compat.v1.variable_scope('D'):
      ...
      biases = tf.compat.v1.get_variable('biases', shape=[n], initializer=tf.zeros_initializer())
      ...
      biases2 = tf.compat.v1.get_variable('biases2', shape=[n], initializer=tf.zeros_initializer())
      hx2=(tf.compat.v1.matmul(ax,weights2)+biases2)
      ax2=tf.compat.v1.nn.relu(hx2)
      ...
      biases3 = tf.compat.v1.get_variable('biases3', shape=[num_units_out], initializer=tf.zeros_initializer())
      hx3=tf.compat.v1.matmul(ax2,weights3)+biases3
      return hx3,tf.compat.v1.nn.sigmoid(hx3)
...
def leaky_relu(x, alpha=0.2):
    return tf.compat.v1.maximum(tf.compat.v1.minimum(0.0, alpha * x), x)
