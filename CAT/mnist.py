...
class MNIST:
        ...
        def load_dataset(self):
                ...
                if self.split=='train':
                        Source_train = pd.read_csv("./data/Target_train.csv")
                        labels = Source_train.labels
                        images = Source_train.drop(['labels'], axis= 1)
                        labels = np.array(labels, dtype=np.int32)
                        images = np.array(images, dtype=np.float32).reshape(-1, 1, 9)########
                        self.images = images
                        self.labels = labels
                elif self.split=='test':
                        Source_test = pd.read_csv("./data/Target_test.csv")
