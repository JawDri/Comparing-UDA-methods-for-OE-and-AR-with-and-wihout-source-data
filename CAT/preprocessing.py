...
def preprocessing(inputs, model_fn):
  ...
  if channels == 1 and model_fn.num_channels == 3:
      ...
      inputs = tf.compat.v1.image.grayscale_to_rgb(inputs)
  elif channels == 3 and model_fn.num_channels == 1:
      print('really RGB TO GREY?')
      inputs = tf.compat.v1.image.rgb_to_grayscale((inputs))
  ...
  if model_fn.default_image_size is not None:
      ...
      inputs = tf.compat.v1.image.resize_images(inputs, [size, size])
      print('after resize ',inputs.get_shape())
  if model_fn.mean is not None:
      logging.info('Performing mean subtraction.')
      inputs = inputs - tf.compat.v1.reshape(tf.compat.v1.constant(model_fn.mean),[-1,3])
...
def rgb2gray(image):
    return tf.compat.v1.reduce_sum(tf.compat.v1.multiply(image, tf.constant(RGB2GRAY)),
                         -1,
                         keep_dims=True)

def gray2rgb(image):
    return tf.compat.v1.multiply(image, tf.compat.v1.constant(RGB2GRAY))
