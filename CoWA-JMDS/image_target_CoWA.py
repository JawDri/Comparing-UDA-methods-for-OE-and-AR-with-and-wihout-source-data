...
def evaluation(loader, netF, netB, netC, args, cnt):
    ...
    F1score = f1_score(all_label.cpu(), torch.squeeze(predict).float().cpu(), average='weighted')
    ...
    args.out_file.flush()
    print(log_str)
    print('F1score = {:.2f}%'.format(F1score * 100) + '\n')
...
def mixup(x, c_batch, t_batch, netF, netB, netC, args):
    ...
    t_batch = torch.eye(args.class_num,  device="cuda")[t_batch.argmax(dim=1)]
    shuffle_idx = torch.randperm(len(x))
    mixed_x = (lam * x.permute(1,0) + (1 - lam) * x[shuffle_idx].permute(1,0)).permute(1,0)
...
def train_target(args):
    ...
    with torch.no_grad():
        ...
        soft_pseudo_label, coeff, accuracy = evaluation(
            dset_loaders["target"], netF, netB, netC, args, cnt
        )
        epochs.append(cnt)
