...
class MemoryBank(nn.Module):
    ...
    def forward(self, inputs, inputs_s, inputs_s1, indexes, k=10):
        ...
        local = (local + _local.expand_as(local))*(torch.eye(B))
        local /= self.temp
        ...
        sim = torch.zeros(labels.max()+1, B).float()  # L * B 
        ...
        nums.index_add_(0, labels, torch.ones(self.num_samples,1).float())
        
        nums_help = torch.zeros(labels.max()+1, 1).float()
        nums_help.index_add_(0, targets, torch.ones(B,1).float())
        ...
        ad_sim_target = torch.zeros(self.args.class_num,B).float()
        ad_sim_target.index_add_(0, target_label, inputs_out.t().contiguous()[target_idx])
        t_nums = torch.zeros(self.args.class_num, 1).float()
        t_nums.index_add_(0, target_label, torch.ones(self.num_samples,1)[target_idx].float())
        ad_sim_source = torch.zeros(self.args.class_num,B).float()
        ad_sim_source.index_add_(0, source_label, inputs_out.t().contiguous()[source_idx])
        s_nums = torch.zeros(self.args.class_num, 1).float()
        s_nums.index_add_(0, source_label, torch.ones(self.num_samples,1)[source_idx].float())
        ...
        if self.args.ad_method == 'LMMD': 
            ...
            batch_help = torch.ones(B)*(-1)+2*batch_mask # target-like 1, source-like -1
